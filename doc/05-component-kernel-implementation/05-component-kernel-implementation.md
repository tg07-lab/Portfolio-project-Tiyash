...
I chose to represent my WorkoutSession using a simple list of WorkoutSet
objects. This representation makes it easy to add and remove sets and to loop
through all sets when needed. It also matches the structure of my kernel methods
like addSet and removeAnySet.For the convention, my list must not be null, and each WorkoutSet must have a
non-empty exercise name, reps greater than 0, and load greater than or equal to
0.For the correspondence, each WorkoutSet in the list represents one set in the
workout session, and the entire list represents the full workout.
...
- Designed kernel implementation for WorkoutSession component
- Implemented WorkoutSession1L using an ArrayList representation
- Added constructor, kernel methods, and Standard methods

### Updated


- Selected and justified representation for kernel implementation
- Added convention and correspondence for WorkoutSession1L
